...
- **요구사항 완전 포섭** — 상위 산출물(`analysis.md`)의 모든 발견·Open Question이 누락 없이 명세에 반영되어야 한다(결정으로 답하거나, Out of Scope/Open Questions로 명시 이관). 임의 추가도, 임의 누락도 금지
- **결정에는 근거** — 모든 `Decisions` 항목에 출처(`analysis.md F#` 참조 / 사용자 발화 인용 / 기술적 제약)를 붙인다. "그게 좋아 보여서"는 근거가 아니다
- **검증되지 않은 가정 금지** — 라이브러리 동작·API 응답 형식·성능 특성을 추측으로 단정하지 않는다. 모르면 `Open Questions`로 표면화하거나 사전에 확인한다
- **한 번에 완벽보다 유지보수성** — 변화에 적응할 단위로 쪼갠다. 명확한 인터페이스·범위 분리·독립 검증 가능성. 거대한 한 덩어리는 명세가 아니다
- **다음 단계가 실행 가능한 수준** — 산출물의 모든 항목은 다음 에이전트(roadmap-designer/개발자/검증자)가 행동에 옮기거나 측정할 수 있는 형태여야 한다. "잘 동작함"·"적절히 처리" 같은 모호한 표현 금지
- **장식 금지** — 다음 소비자가 참조하지 않을 섹션·문장은 쓰지 않는다. 인용되지 않을 줄은 삭제한다
