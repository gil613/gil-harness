...
| `/harness:validate` | 스테이지별 산출물 검증 — REQ/ROADMAP은 추론(서브에이전트), DEV/REVIEW는 결정론(명령어 실행 + 구조 검사) |
...
- **REQUIREMENTS / ROADMAP**: 구조 사전검사(Bash) + **추론 검증** — 검증 서브에이전트(`requirements-validator`, `roadmap-validator`)가 산출물 품질을 판정한다.
- **DEVELOPMENT / REVIEW**: **결정론 검증만** — `typecheckCmd → lintCmd → testCmd → buildCmd` 실행 + 산출물(`progress.md` / `review-report.md`) 구조 검사. 별도 검증 서브에이전트 없음. 코드의 의미 검증은 REVIEW 단계의 `reviewer`가 실제 소스를 읽으며 수행한다 — `progress.md` 산문을 재심사하는 것보다 강한 검사다. REVIEW 결정론 검사는 reviewer가 기록한 판정(Critical/Major finding, Final Verdict)을 파싱해 라우팅한다.

PASS → 다음 스테이지 + `iteration=0` + `failures=[]` + `lastValidated` 갱신.
FAIL → `iteration += 1`, `failures` 배열 끝에 append (최근 20개만 유지). REVIEW에서 코드 수정이 필요한 실패는 DEVELOPMENT로 회귀한다.
