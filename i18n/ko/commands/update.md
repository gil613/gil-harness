...
> Claude Code 플러그인은 이 파일을 사용하지 않습니다 — 정본은 `commands/update.md` (영어)입니다.
...
항상 경로를 큰따옴표로 감싼다. 따옴표 없는 Windows 절대경로(`C:\...`)는 bash가 backslash를 이스케이프로 처리해 현재 폴더에 잘못된 이름의 디렉터리를 만든다.
